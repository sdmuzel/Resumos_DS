...
Esse arquivo contem um resumo sobre SQL, como os comandos basicos necessário para se realizar  uma query. 
Sem se focar em um tipo de "programa" em especifico.

- IBM 
- ALURA


# Introdução a bancos de dados e SQL básico

* SQL: Structured (Inglês) Query Languague
     * Uma linguagem usada para bancos de dados relacionais para consultar dados

    
* Dados: uma coleção de fatos na forma de palavras, números,  imagens ou vídeos
     * Os dados são um dos ativos mais críticos de qualquer empresa.
     * É usado e coletado praticamente em todos os lugares.
     * Seu banco armazena dados sobre você, seu nome, endereço, número de telefone, número da conta, etc.


* Bancos de dados: um repositório de dados
     * Fornece a funcionalidade para adicionar, modificar e consultar esses dados
     * Diferentes tipos de bancos de dados armazenam dados em formatos * diferentes *
     * Os dados armazenados em forma tabular são um banco de dados * relacional *



```
Um banco de dados é um repositório ou coleção de dados logicamente coerente com algum significado inerente
     * Normalmente vem com funcionalidade para adicionar, modificar e consultar dados
     * SQL é comumente usado para acessar dados em bancos de dados relacionais
```


* Banco de dados em nuvem:
     * Vantagens: Facilidade de uso, escalabilidade, recuperação de desastres


* Database Management System (DBMS): um conjunto de ferramentas de software que controla os dados como acesso, organização e armazenamento
   * Termos: DB, servidor de banco de dados e DBMS são frequentemente usados de forma intercambiável
    

* RDBMS (DBMS Relacional):
     * Um conjunto de ferramentas de software para controlar os dados: acesso, organização, armazenamento
     * *Nota*: Um banco de dados relacional armazena dados em um formato tabular, ou seja, em linhas e colunas. Mas nem todos os tipos de banco de dados usam o formato tabular


* Comandos SQL: Exitem dois tipos:
     1. **DDL** - Lingugem de definição de dados (Data Definition Languague): Definir, alterar ou descartar dados (ex: * CREATE)
     2. **DML** - Linguagem de manipulação de dados (Data Manipulation Languague): ler, modificar dados. Explica como o nome da entidade e os atributos são usados para criar uma tabela de banco de dados relacional

Os 5 comandos mais importantes são: `CREATE`, `INSERT`, `SELECT`, `UPDATE`, `DELETE`
...
## Comandos SQL
- OBS 1: Palavras-chave SQL NÃO diferenciam maiúsculas de minúsculas: select é o mesmo que SELECT


- OBS 2: OBS: Alguns sistemas de banco de dados requerem um ponto-e-vírgula no final de cada instrução SQL para execução. 
É uma forma padrão de separar uma instrução SQL de outra, que permite que mais de uma instrução SQL seja executada na mesma chamada para o servidor. 
Portanto, é uma boa prática usar um ponto-e-vírgula no final de cada instrução SQL.

### SELECT  Statement

A instrução SELECT é usada para selecionar dados de um banco de dados.
Os dados retornados são armazenados em uma tabela de resultados, chamada conjunto de resultados (result-set).

1. Recupere os dados da tabela:

Exemplo de uma tabela de banco de dados relacional:
...
FROM book_table  (<tablename>)
```

Podemos recuperar apenas um subconjunto de colunas:
A ordem das colunas na consulta será a ordem de exibição
...
#### Opções para usar junto com o SELECT
...
**SELECT COUNT()**
<br>
A função COUNT() função retorna o número de linhas que correspondem a um critério especificado.
...
Eu ainda posso combinar o DISTICT com o COUNT()
...
Os aliases SQL são usados para dar a uma tabela ou coluna um nome temporário.
...
Para mais exemplos: https://www.w3schools.com/sql/sql_alias.asp


### WHERE clause:

A WHERE é usada para filtrar registros, para extrair apenas os registros que atendem a uma condição especificada. 
- Avalia como: Verdadeiro, Falso ou Desconhecido.

**NOTE**: WHERE não é usada apenas em instruções SELECT, ela também é usada em UPDATE, DELETE, etc.
...
* LIMIT usado para restringir o número de linhas recuperadas do banco de dados
...
A instrução INSERT é usada para adicionar novas linhas a uma tabela. 
É uma das instruções de linguagem de manipulação de dados (DML), usada para ler e modificar dados
...
Se você estiver adicionando valores para todas as colunas da tabela, não será necessário especificar os nomes das colunas na consulta SQL. 
...
Pode-se add mais de uma linha por vez 
...
UPDATE é uma instrução DML e  é usada para modificar os registros existentes em uma tabela.
...
**Nota**: Tenha cuidado ao atualizar os registros em uma tabela! 
Observe a cláusula WHERE, pois ela especifica quais registros devem ser atualizados. 
Se você omitir o WHERE, todos os registros na tabela serão atualizados


### DELETE Statement

A DELETE é uma instrução DML e é usada para excluir registros existentes em uma tabela.
...
- exclui todas as linhas da tabela "Clientes", sem excluir a tabela:
- 
```
...
* modelo de dados mais usado
* permite a independência de dados (dados lógicos / dados físicos / armazenamento físico)
* os dados são armazenados em tabelas, isso fornece independência de dados lógicos, independência de dados físicos e independência de armazenamento físico

Um modelo de dados de relacionamento de entidade, ou modelo de dados ER, é uma alternativa para um modelo de dados relacional. 

O modelo ER é usado como uma ferramenta para projetar bancos de dados relacionais. 
Em modelos ER, entidades são objetos que existem independentemente de quaisquer outras entidades no banco de dados. 
Os blocos de construção dos modelos ER são * entidades * e * atributos *. 
As entidades têm atributos, que são os elementos de dados que caracterizam a entidade.

Um modelo ER propõe pensar em um banco de dados como uma coleção de entidades em vez de ser usado como um modelo por conta própria. 
O modelo ER é usado como uma ferramenta para projetar bancos de dados relacionais. 
No modelo ER, entidades são objetos que existem independentemente de quaisquer outras entidades no banco de dados. 
Uma entidade pode ser um substantivo: pessoa, lugar ou coisa. **Em um diagrama ER, uma entidade é desenhada como um retângulo** 

Entidades têm atributos que são os elementos de dados que caracterizam a entidade. 
Atributos nos dizem mais sobre a entidade. **Em um diagrama ER, os atributos são desenhados como ovais**

EXEMPLO: Biblioteca 
* O livro é um exemplo de uma entidade. 
* A entiadade livro têm atributos como título do livro, a edição do livro, o ano em que o livro foi escrito, etc. 

* Os atributos estão conectados a exatamente uma entidade. 
*O livro de entidades torna-se uma tabela no banco de dados e os atributos se tornam as colunas em uma tabela.

![img_7.png](img_7.png)

Os atributos de entidade serão as colunas das tabelas. A cada tabela é atribuída uma chave primária. 
A chave primária de uma tabela relacional identifica exclusivamente cada tupla ou linha em uma tabela, impedindo a duplicação de dados e fornecendo uma maneira de definir relações entre tabelas. 
Tabelas também podem conter chaves estrangeiras que são chaves primárias definidas em outras tabelas, criando um link entre as tabelas. 
...
* A chave primária de uma tabela relacional identifica exclusivamente cada linha em uma tabela
* O nome e o sobrenome não devem ser nulos
...
existem alguns tipos de dados que podemos utilizar nas tabelas
As mais usadas são:
...
Agora, vamos olhar para um exemplo mais elaborado.
![img_7.png](img_7.png)
Este banco de dados inclui várias entidades, como AUTOR, BOOK, YEAR, etc. 
Vamos começar criando a tabela para a entidade AUTOR. 
O nome da tabela será AUTOR, e seus atributos como AUTOR_ID, FIRSTNAME, LASTNAME, etc. serão as colunas da tabela. 
Nesta tabela, também atribuiremos o atributo author_ID como a Chave Primária, para que nenhum valor duplicado possa existir. 

Lembre-se, a chave primária de uma tabela relacional identifica exclusivamente cada tupla (ou linha) em uma tabela. 
Para criar a tabela Autor, execute o seguinte comando: 

![img_11.png](img_11.png)


Se a tabela que você está tentando criar já existe no banco de dados, você obterá
- um erro indicando "a tabela XXX.YYY já existe". 
- Para contornar este erro, crie uma tabela com um nome diferente ou primeiro DROP a tabela existente
